...
def get_dir_info(dir_path: str) -> Tuple[int, int, int]:
    total_size = 0
    item_count = 0
    file_count = 0
    ...
    return total_size, item_count, file_count

def walk_directory(dir_path: str, script_name: str, output_file: str) -> Iterator[Tuple[str, os.DirEntry, bool, int, int, int]]:
    entries = list(os.scandir(dir_path))
    entries = [e for e in entries if e.name not in {'.git', '.idea', '__pycache__', script_name, output_file}]
    info_cache = {}
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_entry = {executor.submit(get_dir_info, entry.path) if entry.is_dir() else executor.submit(lambda: (entry.stat().st_size, 0, 0)): entry for entry in entries}
        for future in concurrent.futures.as_completed(future_to_entry):
            entry = future_to_entry[future]
            info_cache[entry] = future.result()
    sorted_entries = sorted(entries, key=lambda e: (-info_cache[e][2], -info_cache[e][0], e.name.lower()))
    for i, entry in enumerate(sorted_entries):
        is_last = (i == len(sorted_entries) - 1)
        size, count, file_count = info_cache[entry]
        yield dir_path, entry, is_last, size, count, file_count
        if entry.is_dir():
...
def get_tree_structure(start_path: str, script_name: str, output_file: str) -> str:
    ...
    for dir_path, entry, is_last, size, count, file_count in walk_directory(start_path, script_name, output_file):
...
def main() -> None:
    ...
    with open(output_file, 'w', encoding='utf-8') as out_file:
        ...
        for dir_path, entry, _, size, _, _ in walk_directory(current_dir, script_name, output_file):
            if entry.is_file():
